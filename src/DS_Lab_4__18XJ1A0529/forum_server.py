...
def users(connection, user,num):
    ...
    while True:
        ...
        if data == "/join":
            multicast_Groups[user] = connection
        ...
        if(data[0].isnumeric()):
            ...
            message_send = f"User{num}: {data[1]}".encode('utf-8')
            if user in multicast_Groups.keys():
                for mem in multicast_Groups.keys():
                    multicast_Groups[mem].sendall(message_send)
            else:
                connection.sendall("You're not a member of the multicast group.Enter \'/join\' to join the multicast group.".encode('utf-8'))
